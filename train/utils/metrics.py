...
"""模型验证指标(train11111new 比赛版,原样提取自 utils/metrics.py)。

包含 fitness、ap_per_class、compute_ap、ConfusionMatrix。
"""
...
# 模型 fitness:以 [P, R, mAP@0.5, mAP@0.5:0.95] 加权求和作为模型质量指标
def fitness(x):
...
# 逐类计算平均精度(AP):由精确率-召回率曲线积分得到,可绘制 PR/F1/P/R 曲线
